...
case "$1" in
    ...
    "mysql")
        if [[ -n "$(dockerContainerId mysql)" ]]; then
            ...
            dockerExecMySQL sh -c "MYSQL_PWD=\"${MYSQL_ROOT_PASSWORD}\" mysqldump -h mysql -uroot --opt --single-transaction --events --routines --comments database" | bzip2 > "${BACKUP_DIR}/${BACKUP_MYSQL_FILE}"
            logMsg "Finished"
        else
